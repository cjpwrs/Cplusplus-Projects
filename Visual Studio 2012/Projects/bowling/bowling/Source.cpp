...
int main()
{
	...
	do
	{
		...
		for(int gamecount = 0; gamecount < games; gamecount++)
		{
				...
				//loop for each frame of the game, up to 10 frames
				{
					...
					if(scoretable.size() > 2)
					{
						...
						if(scoretable[location] == "X" && strikespare == true)
						{
							...
							total = striketotal;
							scoretable[location + 2] = int_to_string(striketotal);
						}
						...
						else if( scoretable[location] == "X" && openframe == true && doublestrike == false && triplestrike == false)
						{
							...
							tot = int_to_string(total);
							int updatescore = total - f1 - f2;
							scoretable[location + 2] = int_to_string(updatescore);
						}
						...
						if(scoretable[sparelocation] == "/" && doublespare == true)
						{
							...
							int spare1total = total + 10 + f1;
							total = spare1total;
							scoretable[sparelocation +1] = int_to_string(spare1total);
						}
						...
						if(scoretable[sparelocation] == "/" && sparestrike == true)
						{
							...
							total = sparetotal;
							scoretable[sparelocation + 1] = int_to_string(sparetotal);
						}
						...
						if(scoretable.size() > 5)
						{
							if(scoretable[location] == "X" && scoretable[location - 3] =="X" && openframe == true && triplestrike == false)
							{
								...
								total = total2 + f1 + f2;
								tot = int_to_string(total);
								scoretable[location + 2] = int_to_string(total2);
								scoretable[location - 1] = int_to_string(total1);
							}
						}
							...
							if(frame1 =="X" && scoretable[location] == "X" && scoretable[location - 3] == "X" && scoretable[location - 6] == "X")
							{
								...
								if(lastframe == true)
								{
									...
									if(frame1 == "X" && frame2 == "X")
									{
										...
										total = total3;
										scoretable[location + 2] = int_to_string(total3);
										scoretable[location - 1] = int_to_string(total2);
										scoretable[location - 4] = int_to_string(total1);
									}
									else if(frame1 == "X")
			...
			cout << "Total number of gutterballs: " <<guttercount << endl;
